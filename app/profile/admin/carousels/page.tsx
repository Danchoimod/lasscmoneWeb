...
export default function CarouselPage() {
    ...
    const fetchCarousels = async () => {
        try {
            const res = await fetch("/api-backend/admin/carousels");
            const data = await res.json();
            console.log("Carousel data fetched:", data);
            if (Array.isArray(data)) setCarousels(data);
        } catch (err) {
            console.error(err);
        } finally {
            setLoading(false);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`/api-backend/admin/carousels/${id}`, { method: "DELETE" });
            if (res.ok) fetchCarousels();
        } catch (err) {
            console.error(err);
        }
    };

    if (loading) return <MainLayout><div className="p-8">Loading carousels...</div></MainLayout>;
    ...
    return (
        <MainLayout>
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow-sm border border-gray-100 italic">
                        ...
                        <div key={item.id} className="bg-white rounded-lg border border-gray-100 overflow-hidden shadow-sm group hover:shadow-md transition-shadow">
                            <div className="h-44 bg-gray-50 relative border-b border-gray-50">
                                ...
                                <div className="absolute top-2 right-2 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity z-30">
                                    <Link href={`/carousels/${item.id}/edit`} className="bg-white p-2 rounded shadow text-indigo-600 hover:scale-110 transition-transform font-bold uppercase text-[10px] flex items-center">
                                        Edit
                                    </Link>
